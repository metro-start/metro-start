define(["../pagebase/pagebase_grouped", "../utils/storage", "../utils/util"], (
  ...
  const apps = {
    ...
    setPermissionVisibility: function (visible, cb) {
      let that = this;
      if (visible) {
        chrome.permissions.request({ permissions: ["management"] }, function (
          granted
        ) {
          console.log(`Add management permission: ${granted}`);
          that.enabled = granted;
          cb(granted);
          that.loadApps();
        });
      } else {
        chrome.permissions.remove({ permissions: ["management"] }, function (
          granted
        ) {
          console.log(`Remove management permission: ${granted}`);
          that.enabled = !granted;
          cb(granted);
          that.loadApps();
        });
