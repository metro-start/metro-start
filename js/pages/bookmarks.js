    ...
    setPermissionVisibility: function (visible, cb) {
      ...
      if (visible) {
        chrome.permissions.request({ permissions: ["bookmarks"] }, function (
          ...
          console.log(`Add bookmarks permission: ${granted}`);
          that.enabled = granted;
          cb(granted);
          that.loadBookmarks();
        });
      } else {
        chrome.permissions.remove({ permissions: ["bookmarks"] }, function (
          granted
        ) {
          console.log(`Remove bookmarks permission: ${granted}`);
          that.enabled = !granted;
          cb(granted);
          that.loadBookmarks();
        });
    ...
    templateFunc: function (bookmark) {
      ...
      if (!bookmark.url) {
        titleWrap.firstElementChild.addEventListener(
          "click",
          this.clickBookmark.bind(this, bookmark, titleWrap.firstElementChild)
        );
      ...
      remove.firstElementChild.addEventListener(
        "click",
        this.removeBookmark.bind(this, bookmark, titleWrap.firstElementChild)
      );
